...
class DrawHummingbird:
    def __init__(self, state, window):
        ...
        psi = state[2][0]
        # base
        self.base_points, self.base_index, self.base_meshColors = self.get_box_points(
            self.base_width, self.base_height, self.base_length
        )
        ...
        # counterweight
        self.cw_points, self.cw_index, self.cw_meshColors = self.get_box_points(
            self.cw_width, self.cw_height, self.cw_length
        )
        ...
        # body
        self.body_points, self.body_index, self.body_meshColors = self.get_box_points(
        ...
        # arm
        self.arm_points, self.arm_index, self.arm_meshColors = self.get_box_points(
            self.arm_width, self.arm_height, self.arm_length
        )
        ...
        self.motor_points, self.motor_index, self.motor_meshColors = (
            self.get_box_points(self.motor_width, self.motor_height, self.motor_length)
        )
    ...
    def update(self, t, state):
        ...
        # body
        body_position = cw_position + cw_rotation @ np.array(
            [[self.cw_length / 2 + self.body_length / 2], [0], [0]]
        )
        ...
        # arm
        arm_position = body_position + body_rotation @ np.array(
            [[self.body_length / 2], [0], [0]]
        )
        arm_rotation = Euler2Rotation(phi, theta, psi) @ Euler2Rotation(0, 0, np.pi / 2)
        ...
        # motors
        motor1_position = arm_position + arm_rotation @ np.array(
            [
                [self.arm_length / 2 - self.motor_width / 2],
                [0],
                [-self.arm_height / 2 - self.motor_length / 2],
            ]
        )
        motor1_rotation = Euler2Rotation(phi, theta, psi) @ Euler2Rotation(
            0, np.pi / 2, 0
        )
        ...
        motor2_position = arm_position + arm_rotation @ np.array(
            [
                [-self.arm_length / 2 + self.motor_width / 2],
                [0],
                [-self.arm_height / 2 - self.motor_length / 2],
        ...
        motor2_rotation = Euler2Rotation(phi, theta, psi) @ Euler2Rotation(
            0, np.pi / 2, 0
        )
        ...
        # rotors
        rotor1_position = arm_position + arm_rotation @ np.array(
            [
                [self.arm_length / 2 - self.motor_width / 2],
                [0],
        ...
        rotor1_rotation = Euler2Rotation(phi, theta, psi) @ Euler2Rotation(0, 0, 10 * t)
        ...
        rotor2_position = arm_position + arm_rotation @ np.array(
            [
                [-self.arm_length / 2 + self.motor_width / 2],
                [0],
                [-self.arm_height / 2 - self.motor_length],
        ...
        rotor2_rotation = Euler2Rotation(phi, theta, psi) @ Euler2Rotation(
            0, 0, -10 * t
        )
